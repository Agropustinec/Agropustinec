...
public class TableGenerator {
    ...
    private static final String WORD_SPLIT_REGULAR_EXPRESSIONS = "wordSplit";
    ...
    void printSentenceColumnWidth(StringBuilder table, Text text, int i, int maxDataLength) {
        ...
        StringBuilder sentenceNoPunctuationMarks = noPunctuationMarks(text, i, maxDataLength);
        for (int j = 0; j < differenceSymbol; j++) {
            if (j == differenceSymbol / FIND_MIDDLE) {
                table.append(sentenceNoPunctuationMarks);
            } else {
                table.append(SPACE);
            }
        }
    ...
    public StringBuilder noPunctuationMarks(Text text, int i, int maxDataLength) {
        String[] wordOfSentence = text.getSentences()[i - 1].getContent().split(RegexProvider.get(WORD_SPLIT_REGULAR_EXPRESSIONS));
